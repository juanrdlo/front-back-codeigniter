...
class Utf8_test extends CI_TestCase {
	...
	public function test_is_ascii()
	{
		...
		$this->assertFalse($utf8->is_ascii('Ñ‚ÐµÑÑ‚'));
	}
	...
	public function test_clean_string()
	{
		...
		$illegal_utf8 = "\xc0Ñ‚ÐµÑÑ‚";
		if (MB_ENABLED)
		{
			$this->assertEquals('Ñ‚ÐµÑÑ‚', $utf8->clean_string($illegal_utf8));
		}
		elseif (ICONV_ENABLED)
		{
			// This is a known issue, iconv doesn't always work with //IGNORE
			$this->assertContains($utf8->clean_string($illegal_utf8), array('Ñ‚ÐµÑÑ‚', ''));
		}
		else
		{
	...
	public function test_convert_to_utf8()
	{
		...
		{
			$this->assertEquals('Ñ‚ÐµÑÑ‚', $utf8->convert_to_utf8('òåñò', 'WINDOWS-1251'));
		}
		else
		{
			$this->assertFalse($utf8->convert_to_utf8('òåñò', 'WINDOWS-1251'));
		}
	}
